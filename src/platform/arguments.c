...
static int parse_decimal_as_percentage(const char *str)
{
    ...
    char *end;
    long whole = strtol(start, &end, 10);
    int percentage = 100 * (int) whole;
    if (*end == ',' || *end == '.') {
        end++;
        start = end;
        long fraction = strtol(start, &end, 10);
        switch (end - start) {
            case 0:
                break;
            case 1:
...
int platform_parse_arguments(int argc, char **argv, augustus_args *output_args)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(argv[i], "--display-scale") == 0) {
            if (i + 1 < argc) {
        ...
        } else if (strcmp(argv[i], "--cursor-scale") == 0) {
            if (i + 1 < argc) {
                int percentage = parse_decimal_as_percentage(argv[i + 1]);
                i++;
            ...
            if (i + 1 < argc) {
                output_args->display_id = strtol(argv[i + 1], 0, 10);
                i++;
            } else {
