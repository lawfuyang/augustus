...
void platform_handle_key_down(void *event)
{
    ...
    switch (e->keysym.sym) {
        ...
        case SDLK_KP_ENTER:
            ...
            if ((e->keysym.mod & (KMOD_CTRL | KMOD_ALT | KMOD_GUI)) == 0) {
                keyboard_return();
            }
            break;
        case SDLK_BACKSPACE:
    ...
    // handle hotkeys
    key_type key = get_key_from_scancode(e->keysym.scancode);
    ...
    key_modifier_type mod = get_modifier(e->keysym.mod);
...
void platform_handle_key_up(void *event)
{
    ...
    if ((e->keysym.sym == SDLK_ESCAPE || e->keysym.sym == SDLK_AC_BACK) && !mouse_get()->is_touch) {
